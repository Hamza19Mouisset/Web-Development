...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            HttpResponse('Veuillez confirmer votre email pour completer la creation de votre compte')
    else:
        form = SignUpForm()
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        return HttpResponse('Merci pour la verification de votre compte. Vous pouvez vous connecter')
    else:
...
def Appointmentfun(request):
    if request.method=='POST':
        ...
        try:
            ...
            Allappoints=Appointment.objects.all()
            #for appoint in Allappoints:
            #    if appoint.Date_and_time==Date_and_time:
            #        messages.success(request,"Sorry, we can't accept that!")
            ...
            message = f'Bonjour {request.user.first_name}, Vous avez reserver votre rendez-vous dans le departement: {Medical_Specialties} pour le  { Date_and_time}'
            email_from = settings.EMAIL_HOST_USER
            recipient_list = [request.user.email ]
            send_mail( subject, message, email_from, recipient_list )
            messages.success(request,"Rendez-vous reservé avec succès")
